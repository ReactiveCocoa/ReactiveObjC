...
static void RACSwizzleForwardInvocation(Class baseClass) {
	...
	id newForwardInvocation = ^(id self, NSInvocation *invocation) {
		...
		NSDictionary* interopImpls = objc_getAssociatedObject(baseClass, RACInteropImplsKey);
		...
		if (targetMethod == NULL) {
			...
			if (dynamicImmediateMethod) {
				...
				if (dynamicImmediateImpl != _objc_msgForward) {
					@synchronized (class) {
						// Mutations to `interopImpls` must be made on a a copy and
						// subsequently assigned back, so as to avoid partially updated
						...
						if ([interopImpls objectForKey:key] == nil) {
							NSMutableDictionary* newInteropImpls = [interopImpls mutableCopy];
							[newInteropImpls setObject:[NSValue valueWithPointer:dynamicImmediateImpl]
															 forKey:key];
							objc_setAssociatedObject(class, RACInteropImplsKey, newInteropImpls, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
						}
					}
				}
			}
...
static Class RACSwizzleClass(NSObject *self) {
	...
	if (statedClass != baseClass) {
		...
		@synchronized (swizzledClasses()) {
			if (![swizzledClasses() containsObject:className]) {
				RACSetupInteropImplDictionary(baseClass);
	...
	if (subclass == nil) {
		...
		RACSetupInteropImplDictionary(subclass);
