...
static void RACSwizzleForwardInvocation(Class baseClass, RACTuple *interopImplsInfo) {
	NSDictionary* interopImpls = [interopImplsInfo first];
	pthread_mutex_t* mutex = [[interopImplsInfo second] pointerValue];
		...
		if (targetMethod == NULL) {
			...
			if (dynamicImmediateMethod) {
				...
				if (dynamicImmediateImpl != _objc_msgForward) {
					@synchronized (class) {
						pthread_mutex_t* mutex = [objc_getAssociatedObject(class, RACInteropImplsLockKey) pointerValue];
						NSMutableDictionary* interopImpls = objc_getAssociatedObject(class, RACInteropImplsKey);

#if !NS_BLOCK_ASSERTIONS
						NSCAssert(mutex != nil && interopImpls != nil, @"The interop implementation dictionary and its lock should have been initialized at the time the runtime subclass is swizzled.");
#endif
...
static RACTuple* RACSetupInteropImplDictionary(Class baseClass) {
	...
	@synchronized (baseClass) {
...
#if !NS_BLOCK_ASSERTIONS
		NSCAssert(interopImpls == nil, @"The interop implementation dictionary should be set only once for every runtime subclass.");
#endif
		...
		objc_setAssociatedObject(baseClass, RACInteropImplsLockKey, [NSValue valueWithPointer:mutex], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
	}

	return RACTuplePack(interopImpls, [NSValue valueWithPointer:mutex]);
}
