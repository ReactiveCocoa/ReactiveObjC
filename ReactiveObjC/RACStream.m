...
+ (__kindof RACStream *)empty {
  NSString *reason = [NSString
      stringWithFormat:@"%@ must be overridden by subclasses", NSStringFromSelector(_cmd)];
...
- (__kindof RACStream *)flattenMap:(__kindof RACStream * (^)(id value))block {
  ...
  return [[self bind:^{
    return ^(id value, BOOL *stop) {
      ...
      NSCAssert([stream isKindOfClass:RACStream.class],
                @"Value returned from -flattenMap: is not a stream: %@", stream);
...
- (__kindof RACStream *)combinePreviousWithStart:(id)start
  ...
  return [[[self scanWithStart:RACTuplePack(start)
    ...
    return tuple[1];
  }] setNameWithFormat:@"[%@] -combinePreviousWithStart: %@ reduce:", self.name,
...
- (__kindof RACStream *)reduceEach:(RACReduceBlock)reduceBlock {
  ...
  return [[self map:^(RACTuple *t) {
    NSCAssert([t isKindOfClass:RACTuple.class], @"Value from stream %@ is not a tuple: %@", stream,
              t);
...
+ (__kindof RACStream *)zip:(id<NSFastEnumeration>)streams {
  return [[self join:streams
               block:^(RACStream *left, RACStream *right) {
...
- (__kindof RACStream *)scanWithStart:(id)startingValue
  ...
  return [[self scanWithStart:startingValue
              reduceWithIndex:^(id running, id next, NSUInteger index) {
                return reduceBlock(running, next);
              }] setNameWithFormat:@"[%@] -scanWithStart: %@ reduce:", self.name,
