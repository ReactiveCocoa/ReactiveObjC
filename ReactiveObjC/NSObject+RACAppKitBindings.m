...
- (RACChannelTerminal *)rac_channelToBinding:(NSString *)binding options:(NSDictionary *)options {
  ...
  RACChannelProxy *proxy = [[RACChannelProxy alloc] initWithTarget:self
                                                       bindingName:binding
                                                           options:options];
...
- (instancetype)initWithTarget:(id)target
  ...
  [self.channel.followingTerminal
  ...
  // terminates.
  objc_setAssociatedObject(self.target, (__bridge void *)self, self,
  ...
  RACChannelTo(self, value, options[NSNullPlaceholderBindingOption]) =
      self.channel.followingTerminal;
  return self;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p>{ target: %@, binding: %@ }", self.class, self,
                                    self.target, self.bindingName];
}
