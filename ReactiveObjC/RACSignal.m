...
+ (RACSignal *)startLazilyWithScheduler:(RACScheduler *)scheduler
  ...
  RACMulticastConnection *connection = [[RACSignal createSignal:^id(id<RACSubscriber> subscriber) {
    block(subscriber);
    return nil;
  ...
  return [[[RACSignal createSignal:^id(id<RACSubscriber> subscriber) {
    ...
    [connection connect];
    return nil;
  }] subscribeOn:scheduler] setNameWithFormat:@"+startLazilyWithScheduler: %@ block:", scheduler];
}
...
- (RACSignal *)bind:(RACSignalBindBlock (^)(void))block {
   ...
   * 3. If the binding block returns a signal, subscribe to it, and pass all of its values through
   * to the subscriber as they're received.
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    _Atomic int32_t *signalCount = malloc(sizeof(*signalCount));
    *signalCount = 1;  // indicates self

    RACCompoundDisposable *compoundDisposable = [RACCompoundDisposable compoundDisposable];

    void (^completeSignal)(RACDisposable *) = ^(RACDisposable *finishedDisposable) {
      if (atomic_fetch_sub(signalCount, 1) == 1) {
    ...
    @autoreleasepool {
      ...
      RACDisposable *bindingDisposable = [self
          subscribeNext:^(id x) {
...
- (RACDisposable *)subscribeError:(void (^)(NSError *))errorBlock
                        completed:(void (^)(void))completedBlock {
  NSCParameterAssert(completedBlock != NULL);
  NSCParameterAssert(errorBlock != NULL);

  RACSubscriber *o = [RACSubscriber subscriberWithNext:NULL
                                                 error:errorBlock
                                             completed:completedBlock];
  return [self subscribe:o];
}
...
- (id)asynchronousFirstOrDefault:(id)defaultValue
  ...
  [[[[self take:1] timeout:timeout
               onScheduler:[RACScheduler scheduler]] deliverOn:RACScheduler.mainThreadScheduler]
      subscribeNext:^(id x) {
        result = x;
        done = YES;
      }
