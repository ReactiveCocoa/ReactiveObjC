...
static CFComparisonResult RACCompareScheduledActions(const void *ptr1, const void *ptr2,
                                                     void *info) {
...
static void RACReleaseScheduledAction(CFAllocatorRef allocator, const void *ptr) { CFRelease(ptr); }
...
- (instancetype)init {
  ...
  CFBinaryHeapCallBacks callbacks = (CFBinaryHeapCallBacks){.version = 0,
  ...
  @synchronized(self) {
    for (NSUInteger i = 0; i < ticks; i++) {
      ...
      if (previousScheduler != nil) {
        NSThread.currentThread.threadDictionary[RACSchedulerCurrentSchedulerKey] =
            previousScheduler;
      } else {
        [NSThread.currentThread.threadDictionary
            removeObjectForKey:RACSchedulerCurrentSchedulerKey];
      }
    }
  }
}
...
- (RACDisposable *)schedule:(void (^)(void))block {
  ...
  @synchronized(self) {
    NSDate *uniqueDate =
        [NSDate dateWithTimeIntervalSinceReferenceDate:self.numberOfDirectlyScheduledBlocks];
    self.numberOfDirectlyScheduledBlocks++;

    RACTestSchedulerAction *action = [[RACTestSchedulerAction alloc] initWithDate:uniqueDate
                                                                            block:block];
...
- (RACDisposable *)after:(NSDate *)date
  ...
  @synchronized(self) {
    ...
    void (^reschedulingBlock)(void) = ^{
      ...
      RACDisposable *schedulingDisposable = [self after:[date dateByAddingTimeInterval:interval]
    ...
    RACTestSchedulerAction *action =
        [[RACTestSchedulerAction alloc] initWithDate:date block:reschedulingBlock];
    CFBinaryHeapAddValue(self.scheduledActions, (__bridge void *)action);
