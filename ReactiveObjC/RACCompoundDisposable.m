...
- (instancetype)initWithDisposables:(NSArray *)otherDisposables {
...
#if RACCompoundDisposableInlineCount
  [otherDisposables
      enumerateObjectsUsingBlock:^(RACDisposable *disposable, NSUInteger index, BOOL *stop) {
  ...
  if (otherDisposables.count > RACCompoundDisposableInlineCount) {
    ...
    CFRange range = CFRangeMake(RACCompoundDisposableInlineCount,
                                (CFIndex)otherDisposables.count - RACCompoundDisposableInlineCount);
  ...
  pthread_mutex_lock(&_mutex);
  {
    if (_disposed) {
      ...
      if (RACCOMPOUNDDISPOSABLE_ADDED_ENABLED()) {
        RACCOMPOUNDDISPOSABLE_ADDED(
            self.description.UTF8String, disposable.description.UTF8String,
            CFArrayGetCount(_disposables) + RACCompoundDisposableInlineCount);
      }
