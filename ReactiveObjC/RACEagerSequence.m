...
+ (RACSequence *)return:(id)value {
  return [[self sequenceWithArray:@[ value ]
                           offset:0] setNameWithFormat:@"+return: %@", RACDescription(value)];
}
...
- (RACSequence *)bind:(RACSequenceBindBlock (^)(void))block {
  ...
  return [[self.class sequenceWithArray:resultArray
                                 offset:0] setNameWithFormat:@"[%@] -bind:", self.name];
}
...
- (RACSequence *)concat:(RACSequence *)sequence {
  ...
  return [[self.class sequenceWithArray:array offset:0]
...
- (id)foldRightWithStart:(id)start reduce:(id (^)(id, RACSequence *rest))reduce {
  return [super foldRightWithStart:start
                            reduce:^(id first, RACSequence *rest) {
