  ...
  @synchronized(swizzledClasses()) {
    ...
    id newDealloc = ^(__unsafe_unretained id self) {
      RACCompoundDisposable *compoundDisposable =
          objc_getAssociatedObject(self, RACObjectCompoundDisposable);
      [compoundDisposable dispose];

      if (originalDealloc == NULL) {
        struct objc_super superInfo = {.receiver = self,
    ...
    if (!class_addMethod(classToSwizzle, deallocSelector, newDeallocIMP, "v@:")) {
      ...
      originalDealloc =
          (__typeof__(originalDealloc))method_setImplementation(deallocMethod, newDeallocIMP);
    }
...
- (RACCompoundDisposable *)rac_deallocDisposable {
  @synchronized(self) {
    ...
    objc_setAssociatedObject(self, RACObjectCompoundDisposable, compoundDisposable,
