...
static _Atomic int DEALLOC_OVERFLOW_GUARD = 100;
...
- (void)dealloc {
  static _Atomic int32_t directDeallocCount = 0;

  if (atomic_fetch_add(&directDeallocCount, 1) >= DEALLOC_OVERFLOW_GUARD) {
    atomic_fetch_add(&DEALLOC_OVERFLOW_GUARD, -directDeallocCount);
...
- (NSString *)description {
  ...
  return [NSString stringWithFormat:@"<%@: %p>{ name = %@, head = %@, tail = %@ }", self.class,
                                    self, self.name, head, tail];
}
