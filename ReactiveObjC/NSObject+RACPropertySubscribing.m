...
- (RACSignal *)rac_valuesForKeyPath:(NSString *)keyPath observer:(__weak NSObject *)observer {
  return [[[self rac_valuesAndChangesForKeyPath:keyPath
                                        options:NSKeyValueObservingOptionInitial
                                       observer:observer] map:^(RACTuple *value) {
...
- (RACSignal *)rac_valuesAndChangesForKeyPath:(NSString *)keyPath
  ...
  RACSignal *deallocSignal = [[RACSignal zip:@[
    self.rac_willDeallocSignal, strongObserver.rac_willDeallocSignal ?: [RACSignal never]
  ]] doCompleted:^{
  ...
  return [[[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
    ...
    return [self rac_observeKeyPath:keyPath
                            options:options
                           observer:observer
      ...
      setNameWithFormat:@"%@ -rac_valueAndChangesForKeyPath: %@ options: %lu observer: %@",
                        RACDescription(self), keyPath, (unsigned long)options,
                        RACDescription(strongObserver)];
}
